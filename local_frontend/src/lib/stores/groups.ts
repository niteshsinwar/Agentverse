...
import type { Group, Agent, Message, Document } from '../types';
...
export const useGroupsStore = create<GroupsStore>((set, get) => ({
  ...
  loadGroupAgents: async (groupId: string) => {
    set((state) => ({
      groupAgents: { ...state.groupAgents, loading: true, error: null },
    }));
  ...
  uploadDocument: async (
    ...
    try {
      const document = await groupsApi.uploadDocument(
        groupId,
        { file, agent_id: agentId, message, target_agent: agentId, sender: 'user' },
        onProgress
      );
        ...
        try {
          ...
          switch (data.type) {
            ...
            case 'tool_call':
              ...
              // Filter out 'reflect' tool calls - these are intrinsic agent capabilities
              // shown via agent_thought events, not explicit tool calls
              if (data.payload.tool === 'reflect') {
                console.log('Skipping reflect tool_call (shown as agent_thought instead)');
                break;
              }
